...
### リンク比較をゆるく変更
- リンクのマッチング条件からlabel（何が/何を/何に/どこで/いつ）の比較を削除
- relationshipのLLM調整後の値のみで比較するよう変更
- これにより、異なるlabelを持っていてもrelationshipが同じならマッチするようになった
- 変更ファイル:
  - `workers/src/services/comparison-service.ts` - リンクマッチングロジック変更（labelMatches条件削除）
- コミット: 21be376
...
- 類似度スコア: 52.8%（見本マップ vs 学生マップ）
- 一致ノード: 9個、一致リンク: 5個
- 学生に不足している概念・命題が正しく表示されている
- LLMによる語彙調整（日光→光エネルギー等）も正常動作

### 比較ロジックでのリンクlabel比較を追加（2025-12-09）
- `workers/src/services/comparison-service.ts`のリンクマッチングロジックを修正
- 従来: `adjustedRelationship`のみで比較していた
- 修正後: `link.label`（何が/何を/何に/どこで/いつ）も比較対象に追加
- これにより、異なるlabelを持つリンクが誤ってマッチする問題を解消
- コミット: 3d7f49f
...
4. 統合テストの実施
5. Cloudflare Workersへのデプロイ（要CLOUDFLARE_API_TOKEN）
