...
- リンクは必ず**動詞ノード → 名詞ノード**の方向でのみ作成可能
  - 名詞ノード同士を直接リンクすることはできない
  - 動詞ノード同士を直接リンクすることはできない

### 5.2 ノードの制約
- **名詞ノード**: 同一概念マップ内でラベルの重複は禁止（大文字小文字を区別しない）
  - 理由: 同じ概念は一つのノードで表現し、共有すべきである
- **動詞ノード**: ラベルの重複は許可
  - 理由: 動詞ノードはリンク先の名詞ノードで意味が区別されるため
